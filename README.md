...
11. **Ayurveda AI: Medicinal Plant Identifier**
    ...
    * **How itWorks:** User uploads a plant picture. The AI identifies it and details its Ayurvedic properties.
...
* **Text-to-Speech:** AI-generated responses in modules and the assistant can be read aloud. The `speak()` function:
    1.  Calls the Gemini TTS API with the text and selected language.
    2.  Receives raw PCM audio data.
    3.  Converts the PCM data into a playable WAV blob in the browser.
    4.  Manages an `Audio` object to play the sound and update the UI state.
